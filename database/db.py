...
def seed_db(conn):
    ...
    # Pastikan akun admin selalu terbarui walaupun email sudah pernah dibuat sebagai user biasa.
    for name, email, password in admin_accounts:
        admin_hash = generate_password_hash(password)
        if IS_POSTGRES:
            cur.execute(
                "UPDATE users SET name=%s, password_hash=%s, role=%s WHERE lower(email)=lower(%s)",
                (name, admin_hash, "admin", email),
            )
        else:
            cur.execute(
                "UPDATE users SET name=?, password_hash=?, role=? WHERE lower(email)=lower(?)",
                (name, admin_hash, "admin", email),
            )
